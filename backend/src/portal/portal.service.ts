...
export class PortalService {
  ...
  async login(identityNumber: string, birthDate: string) {
    ...
    if (!worker) throw new UnauthorizedException('Trabajador no encontrado');

    if (!worker.tenant.hasWorkerPortalAccess) {
      throw new ForbiddenException('Su empresa no cuenta con este servicio habilitado. Contacte a Recursos Humanos.');
    }

    if (!worker.tenant.isActive || (worker.tenant.serviceEndDate && new Date(worker.tenant.serviceEndDate).getTime() < Date.now())) {
      throw new ForbiddenException('Su empresa presenta suspensión de los servicios corporativos en línea.');
    }
  ...
  async getReceipts(workerId: string) {
  ...
  async getReceiptByToken(token: string) {
    const receipt = await this.prisma.payrollReceipt.findFirst({
      where: { 
        OR: [
          { signatureToken: token }, 
          { id: token } // Retrocompatibilidad para recibos sin token
        ] 
      },
      include: {
        worker: {
  ...
  async signReceipt(id: string, ipAddress: string) {
  ...
  async getSelfServiceDocuments() {
    // Only return documents that have isSelfService = true. We don't filter by tenantId 
    // because this route does not use TenantMiddleware. Wait, we do need to filter by tenantId!
    // But since the portal login returns workerId, we can find the worker's tenantId.
    // Instead, the controller could receive the workerId parameter to contextually scope it.
    // Actually, let's just make the user pass workerId to all these endpoints.
    return [];
  }

  async getSelfServiceDocumentsByWorker(workerId: string) {
  ...
  async compileSelfServiceDocument(templateId: string, workerId: string) {
    const worker = await this.prisma.worker.findUnique({ where: { id: workerId } });
    if (!worker) throw new NotFoundException('Trabajador no encontrado');

    const template = await this.prisma.documentTemplate.findUnique({ where: { id: templateId } });
    if (!template || !template.isSelfService) throw new UnauthorizedException('Plantilla no disponible para autogestión');

    return this.documentTemplatesService.compile(worker.tenantId, templateId, workerId);
  }

  // Requerimientos (Tickets del Help Desk)
  async getTickets(workerId: string) {
  ...
  async createTicket(workerId: string, data: any) {
    const worker = await this.prisma.worker.findUnique({ where: { id: workerId } });
    if (!worker) throw new NotFoundException('Trabajador no encontrado');
  ...
  async addTicketComment(workerId: string, ticketId: string, commentText: string) {
    const worker = await this.prisma.worker.findUnique({ where: { id: workerId } });
    if (!worker) throw new NotFoundException('Trabajador no encontrado');
  ...
  async getLoansAccount(workerId: string, currencyView: string, exchangeRateString: string) {
