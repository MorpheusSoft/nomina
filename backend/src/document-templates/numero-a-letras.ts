export function numeroALetras(num: number, currency: 'USD' | 'VES'): string {
    ...
    else if (enteros === 1) letrasEnteros = Millones(enteros) + ' ' + singular;
    else letrasEnteros = Millones(enteros) + ' ' + plural;
    ...
    let letrasCentavos = '';
    if (centavos > 0) {
        if (centavos === 1) letrasCentavos = 'con un ' + centsSingular;
        else letrasCentavos = 'con ' + Decenas(centavos) + ' ' + centsPlural;
    } else {
